...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    locations.forEach((loc) => {
      ...
      el.src = "https://studyspotr.s3.us-east-2.amazonaws.com/Vector.png";
      el.style.width = "22px";
      el.style.height = "30px";
      el.style.cursor = "pointer";

      new mapboxgl.Marker({ element: el })
        .setLngLat([loc.location.long, loc.location.lat])
        .addTo(map.current);
    });
  }, [userLocation, theme]);
  ...
  const locations = [
    ...
    { name: "Cafe", location: { lat: 49.05, long: -122.789 } },
    { name: "Quiet Study Room", location: { lat: 49.049, long: -122.781 } },
  ];
  ...
  return (
    <>
      <div
        id="map"
        className={`
          w-screen h-screen transition-all duration-500 ease-in-out
          ${view === "map" ? "ml-0" : "-ml-[14vw]"}
        `}
      >
